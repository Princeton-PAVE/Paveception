...
for i in range(1,6):
    ...
    img1 = cv2.imread(f"./photos/img{i}a.jpg")
    img2 = cv2.imread(f"./photos/img{i}b.jpg")
    ...
    src_pts_accum.extend([kp1[m.queryIdx].pt for m in good_matches])
    dst_pts_accum.extend([kp2[m.trainIdx].pt for m in good_matches])

src_pts_accum = np.float32(src_pts_accum).reshape(-1, 1, 2)
dst_pts_accum = np.float32(dst_pts_accum).reshape(-1, 1, 2)
# 4. Find Homography
H, mask = cv2.findHomography(src_pts_accum, dst_pts_accum, cv2.RANSAC, 5.0)
